...
switch("passL", "-lm")
switch("passL", "-lgcc")
switch("passL", "-L" & sdkBase / "components/esp8266/lib")
switch("passL", "-lnet80211 -lpp -lphy -lhal -lcore -lrtc -lclk -lsmartconfig")

task flash, "Convert to bin and flash to ESP8266":
  let sdkPath = sdkBase
  let esptool = sdkPath / "components/esptool_py/esptool/esptool.py"
  exec "python3 " & esptool & " --chip esp8266 elf2image --flash_mode dio --flash_freq 40m --flash_size 4MB test"
  exec "python3 " & esptool & " --chip esp8266 --port /dev/ttyUSB0 --baud 115200 write_flash 0x0 test-0x00000.bin 0x10000 test-0x10000.bin"
